...
class CSVec(object):
    ...
    def compress(self, vec: torch.Tensor):
        self.accumulateVec(vec)
        return self.table

    def uncompress(self, table: torch.Tensor, k=None, epsilon=None):
        assert(table.size() == self.table.size())
        self.table = table
        return self.unSketch(k, epsilon)